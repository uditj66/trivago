...
export const loader = async ({ params }: LoaderFunctionArgs) => {
  ...
  const [trip, allUserstrips] = await Promise.all([
    getTripById(tripId),
    getAllTrips(4, 0),
  ]);
  ...
  return {
    trip,
    allTrips: allUserstrips.trips.map(({ $id, tripDetails, imageUrls }) => ({
      id: $id,
...
const TravelDetail = ({ loaderData }: Route.ComponentProps) => {
  const imageUrls = (loaderData as any).trip?.imageUrls || [];
  const tripData = parseTripData((loaderData as any).trip.tripDetails);
  const paymentLink = (loaderData as any).trip.payment_link;
